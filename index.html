...
<body class="bg-gray-100 font-sans antialiased">
            ...
            <div id="adminTab" class="tab-content hidden">
                <div class="bg-white rounded-xl p-4 mb-4">
                    <div class="flex justify-between items-center">
                        <h3 class="font-bold text-lg">👨‍✈️ Drivers</h3>
                        <button id="addDriverBtn" class="bg-blue-500 text-white px-3 py-1 rounded-lg">+ Add</button>
                    </div>
                ...
                <div class="bg-white rounded-xl p-4">
                    ...
                    <div class="flex gap-2 mt-2">
                        <input id="newStaffEmail" placeholder="Email" class="border p-2 rounded flex-1">
                        <input id="newStaffPassword" placeholder="Password" class="border p-2 rounded flex-1">
                        <button id="createStaffBtn" class="bg-blue-600 text-white px-4 rounded">Add Staff</button>
                    </div>
                    <div id="staffList" class="mt-3 space-y-2 text-sm"></div>
                </div>
                <div class="bg-white rounded-xl p-4 mt-4">
                    <button id="exportAllReportsBtn" class="bg-purple-700 text-white w-full py-2 rounded-xl">📄 Export Full Report (CSV)</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div id="productModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl max-w-md w-full p-5">
            <h3 class="text-xl font-bold mb-3" id="modalTitle">Add Product</h3>
            <input id="prodName" placeholder="Product Name" class="w-full border p-2 rounded mb-2">
            <input id="prodSku" placeholder="SKU (optional)" class="w-full border p-2 rounded mb-2">
            <input id="prodStock" type="number" placeholder="Current Stock" class="w-full border p-2 rounded mb-2">
            <input id="prodThreshold" type="number" placeholder="Low Stock Threshold" class="w-full border p-2 rounded mb-2">
    ...
    <div id="deliveryModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl max-w-md w-full p-5">
            <h3 class="text-xl font-bold mb-2" id="deliveryModalTitle">New Stock-In</h3>
            <select id="deliveryType" class="w-full border p-2 rounded mb-2">
            ...
            <div id="outgoingFields" class="hidden">
                ...
                <input id="customerAddress" placeholder="Address" class="w-full border p-2 rounded mb-2">
                <select id="driverId" class="w-full border p-2 rounded mb-2"><option value="">Assign Driver</option></select>
                <select id="deliveryStatus" class="w-full border p-2 rounded mb-2">
                    <option value="pending">Pending</option>
                    <option value="delivered">Delivered</option>
                    <option value="failed">Failed</option>
                </select>
                <label class="block text-sm">📸 Proof Photo (optional)</label>
                <input type="file" id="proofPhoto" accept="image/*" class="border p-1 rounded w-full mb-2">
            </div>
            <div class="flex gap-2">
    ...
    <div id="driverModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50 p-4">
        <div class="bg-white rounded-2xl p-5 w-80">
            <h3 class="font-bold mb-3">Add Driver</h3>
            <input id="driverName" placeholder="Driver Name" class="w-full border p-2 rounded mb-3">
            <div class="flex gap-2">
                <button id="confirmDriverBtn" class="bg-blue-600 text-white px-4 py-2 rounded flex-1">Add</button>
                <button id="closeDriverModal" class="bg-gray-400 px-4 py-2 rounded flex-1">Cancel</button>
            </div>
        </div>
    </div>
    ...
    <script type="module">
        ...
        import { getFirestore, collection, doc, getDocs, addDoc, updateDoc, deleteDoc, query, where, onSnapshot, enableIndexedDbPersistence, Timestamp, increment, orderBy, getDoc, writeBatch, setDoc } from "firebase/firestore";
        ...
        async function setUserRole(user) {
            ...
            if (user.email === "admin@deliverstock.com") userRole = "admin";
        ...
        function renderProducts() {
            ...
            container.innerHTML = filtered.map(p => `
                <div class="bg-white p-4 rounded-xl shadow flex flex-wrap justify-between items-center">
                    ...
                    <span class="text-2xl font-mono">📦 ${p.currentStock}</span> ${p.currentStock <= p.lowThreshold ? '<span class="bg-red-500 text-white text-xs px-2 py-1 rounded-full low-stock-badge">LOW</span>' : ''}
                    <div class="text-sm">Threshold: ${p.lowThreshold}</div></div>
                    <div class="flex gap-2 mt-2">${userRole === 'admin' ? `<button class="edit-product bg-blue-500 text-white px-3 py-1 rounded" data-id="${p.id}">✏️ Edit</button><button class="delete-product bg-red-500 text-white px-3 py-1 rounded" data-id="${p.id}">🗑️ Delete</button>` : ''}
        ...
        function openEditProduct(id) {
            ...
            if(p){
                ...
                document.getElementById('prodThreshold').value=p.lowThreshold;
        ...
        async function saveProduct() {
            ...
            if(!name || isNaN(stock) || isNaN(threshold)) { alert("Fill all fields"); return; }
            if(window._editId) {
                await updateDoc(doc(db,"products",window._editId), {name, sku, currentStock: stock, lowThreshold: threshold});
            } else {
                await addDoc(collection(db,"products"), {name, sku, currentStock: stock, lowThreshold: threshold});
            }
            closeProductModal();
        }
        ...
        function updateDeliveryProductSelects() {
            const select = document.getElementById('deliveryProductId');
            if(select) select.innerHTML = productsCache.map(p => `<option value="${p.id}">${p.name} (Stock: ${p.currentStock})</option>`).join('');
        }

        function updateDriverSelect() {
            const driverSelect = document.getElementById('driverId');
            if(driverSelect) driverSelect.innerHTML = '<option value="">No Driver</option>' + driversCache.map(d => `<option value="${d.id}">${d.name}</option>`).join('');
        }
        ...
        function openQuickStockIn(prodId) {
            document.getElementById('deliveryModalTitle').innerText = "Stock-In";
            document.getElementById('deliveryType').value = "in";
            document.getElementById('deliveryProductId').value = prodId;
            document.getElementById('outgoingFields').classList.add('hidden');
        ...
        function renderDeliveries() {
            ...
            container.innerHTML = filtered.map(d => {
                ...
                return `<div class="bg-white p-4 rounded-xl shadow"><div class="flex justify-between"><span class="font-bold">${d.type === 'in' ? '📥 Stock-In' : '🚚 Delivery'}</span><span class="text-xs bg-gray-200 px-2 py-1 rounded">${dateStr}</span></div>
                <p class="mt-2">Product: <strong>${prod?.name || 'Unknown'}</strong> | Qty: <strong>${d.quantity}</strong></p>
                ${d.type === 'out' ? `<p>Customer: ${d.customerName || '-'} | Driver: ${driver?.name || 'Unassigned'}</p><select class="status-change mt-2 w-full border p-1 rounded" data-id="${d.id}"><option value="pending" ${d.status === 'pending' ? 'selected' : ''}>Pending</option><option value="delivered" ${d.status === 'delivered' ? 'selected' : ''}>Delivered</option><option value="failed" ${d.status === 'failed' ? 'selected' : ''}>Failed</option></select>` : ''}
                </div>`;
            }).join('');
            document.querySelectorAll('.status-change').forEach(select => select.addEventListener('change', async (e) => {
                ...
                else if(newStatus === 'pending' && delivery.status === 'failed') {
                    const prodRef = doc(db,"products",delivery.productId);
                    await updateDoc(prodRef, { currentStock: increment(-delivery.quantity) });
                }
        ...
        async function addDriver() {
            ...
            if(name) await addDoc(collection(db,"drivers"),{name});
        ...
        function renderDriversList() {
            ...
            if(container) container.innerHTML = driversCache.map(d => `<div class="flex justify-between p-2 bg-gray-50 rounded"><span>${d.name}</span><button class="text-red-500 text-sm" onclick="alert('Driver deletion not yet implemented')">Remove</button></div>`).join('');
        }

        // CSV Export
        async function exportFullReports() {
            let csvRows = [["Type","Product","Quantity","Customer","Driver","Status","Date"]];
            deliveriesCache.forEach(d => {
                const prod = productsCache.find(p=>p.id===d.productId);
                const driver = driversCache.find(dr=>dr.id===d.driverId);
                const dateStr = d.timestamp?.toDate?.() ? d.timestamp.toDate().toLocaleString() : new Date(d.timestamp).toLocaleString();
                csvRows.push([d.type, prod?.name || '-', d.quantity, d.customerName || '-', driver?.name || '-', d.status || '-', dateStr]);
            });
            const ws = XLSX.utils.aoa_to_sheet(csvRows);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "Reports");
            XLSX.writeFile(wb, "stockflow-reports.csv");
        }
        ...
        // Delivery modal
        document.getElementById('quickStockInBtn').onclick = () => {
            document.getElementById('deliveryModalTitle').innerText='Stock-In';
            document.getElementById('deliveryType').value='in';
            document.getElementById('outgoingFields').classList.add('hidden');
            document.getElementById('deliveryModal').classList.remove('hidden');
        };
        document.getElementById('quickDeliveryBtn').onclick = () => {
            document.getElementById('deliveryModalTitle').innerText='Outgoing Delivery';
            document.getElementById('deliveryType').value='out';
            document.getElementById('outgoingFields').classList.remove('hidden');
            document.getElementById('deliveryModal').classList.remove('hidden');
        };
        document.getElementById('deliveryType').onchange = () => {
            const type = document.getElementById('deliveryType').value;
            document.getElementById('outgoingFields').classList.toggle('hidden', type === 'in');
        };
        ...
        document.getElementById('filterDeliveryStatus').addEventListener('change', renderDeliveries);

        document.getElementById('exportProductsCsv').onclick = () => {
            const ws = XLSX.utils.json_to_sheet(productsCache.map(p=>({Name:p.name,SKU:p.sku||'-',Stock:p.currentStock,Threshold:p.lowThreshold})));
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "Products");
            XLSX.writeFile(wb, "stockflow-products.csv");
        };
        ...
        document.getElementById('createStaffBtn').onclick = async () => {
            ...
            const pwd=document.getElementById('newStaffPassword').value;
            if(!email || !pwd) return alert("Email & password required");
            try {
                const userCred = await createUserWithEmailAndPassword(auth, email, pwd);
                await setDoc(doc(db, "users", userCred.user.uid), {role: "staff", email});
                alert("Staff created");
        ...
        console.log("StockFlow App Ready. Update Firebase config for full sync.");
    </script>
</body>
</html>
